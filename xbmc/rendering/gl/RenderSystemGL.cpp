...
void CRenderSystemGL::PushRoundedClip(float radius, float width, float height)
{
  m_roundedClipStack.push({radius, width, height});
}
...
void CRenderSystemGL::ApplyRoundedClipUniforms(const CRect& atlasBounds)
{
  if (m_method != ShaderMethodGL::SM_TEXTURE_ROUNDED || m_roundedClipStack.empty()) return;
  if (!m_pShader[m_method]) return;

  const auto& state = m_roundedClipStack.top();
  glEnable(GL_BLEND);

  if (m_locDimensions >= 0)
    glUniform2f(m_locDimensions, state.width, state.height);

  if (m_locRoundedRadius >= 0)
    glUniform1f(m_locRoundedRadius, state.radius);

  // Pass the atlas boundaries
  if (m_locAtlasBounds >= 0)
    glUniform4f(m_locAtlasBounds, atlasBounds.x1, atlasBounds.y1, atlasBounds.x2, atlasBounds.y2);
}
...
void CRenderSystemGL::InitialiseShaders()
{
  ...
  {
    GLuint prog = m_pShader[ShaderMethodGL::SM_TEXTURE_ROUNDED]->ProgramHandle();
    m_locDimensions = glGetUniformLocation(prog, "m_dimensions");
    m_locRoundedRadius = glGetUniformLocation(prog, "m_radius");
    m_locAtlasBounds = glGetUniformLocation(prog, "m_atlasBounds");
  }
}
