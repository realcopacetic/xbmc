...
bool CGraphicContext::SetClipRegionStencilRounded(float x, float y, float w, float h, float radiusGui)
{
  if (w <= 0.0f || h <= 0.0f)
    return false;

  // Convert GUI logical coords (top-left) into final screen coords using the same scaler as scissor.
  float x1 = x,     y1 = y;
  float x2 = x + w, y2 = y + h;
  float z  = 0.0f;
  ...
  // Radius must be in the same coord space as rectScreenTL (post ScaleFinalCoords()).
  // This intentionally captures ONLY the GUI->screen scaling, not any animation transforms.
  float radiusScreen = 0.0f;
  if (radiusGui > 0.0f)
  {
    // Measure scale by transforming a delta; translation cancels out.
    float ax0 = 0.0f, ay0 = 0.0f;
    float ax1 = radiusGui, ay1 = radiusGui;
    float az = 0.0f;
    ...
    // Use the conservative axis to avoid overshooting on non-uniform scaling.
    radiusScreen = radiusGui * std::min(scaleX, scaleY);
  }

  return CServiceBroker::GetRenderSystem()->BeginStencilClip(rectScreenTL, radiusScreen);
}
